...
def run_mcmc2(pars,data,corr_mat,prior,chifun,file,nstep=5000):
	...
	prior_chi =  ((pars[3]-prior[0])/prior[1])**2
	file = open(file,'w')
	npar=len(pars)
	chain=np.zeros([nstep,npar])
	chivec=np.zeros(nstep)
	y_pred = get_spectrum(pars,len(data[0]))
	chi_cur=chifun(data[1],y_pred,data[2])+prior_chi
	L = 0.7*np.linalg.cholesky(corr_mat)	
	for i in range(nstep):
		dpar = L@np.random.randn(npar)
		pars_trial=pars+dpar
#		pars_trial[3] = prior[0]+prior[1]*np.random.randn() # Sampling selectively for tau separately
#		pars[3] = prior[0] 			# Shifting the center to 0.0544 
#		dpar[3] = prior[1]*np.random.randn()*np.sign(dpar[3])  # Scaling the tau parameter to given sigma 		
		print('\nStep ',i,'  Trial parameter: \n',pars_trial)
		if pars_trial[3]<0.0:
			continue
		y_new = get_spectrum(pars_trial,len(data[0]))		
		prior_chi =  ((pars_trial[3]-prior[0])/prior[1])**2		
		chi_trial=chifun(data[1],y_new,data[2])+prior_chi
...
chain,chivec=run_mcmc2(fun,pars_guess,[x,y,sig],step_size,chifun_sinh,file)

pars_guess=np.median(chain,axis=0)
step_size_better=np.std(chain[chain.shape[0]//10:,:],axis=0)
chain2,chivec2=run_mcmc2(fun,pars_guess,[x,y,sig],step_size_better,chifun_sinh,file,20000)
...
chain3,chivec3=run_mcmc(fun,pars_guess,[x,y,sig],mycov,chifun_sinh,file,20000)
