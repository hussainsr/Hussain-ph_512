...
#-- Running all parameters from current
#pars_cur=np.asarray([65,0.02,0.1,0.05,2e-9,0.96])
delete_index = []
lmd = 0.0
pars_cur2, lhs2, chi2 = LM(pars_cur,y_data,error_data,delete_index,f,lmd)
...
delete_index = []
lmd = 0.0
pars_cur3, lhs3, chi3 = LM(pars_cur,y_data,error_data,delete_index,f,lmd)
...
#-- Part c -------------------------------------------------------------------
# Choosing multipole, lets say l = 400
pars_cur = np.asarray([7.08639629e+01,2.21988148e-02,1.07111919e-01,5.00000000e-02,1.98762615e-09,9.70782148e-01])
def fun(pars,i):
	p = 1000
	x = np.linspace(pars_cur[i]*0.999,pars[i]*1.001,11)
	n = p + 10 # Redundant varuable, needed to run get_spectrum
	y = np.zeros(len(x))
	pars_new = pars.copy()
	for j in range(len(x)):
		pars_new[i] = x[j]
		y[j] = get_spectrum(pars_new,n)[p]
	dx = pars[i]/1000.0
	par = copy.deepcopy(pars)
	par[i] += dx
	y2=get_spectrum(par,n)[p]
	par[i] -= 2*dx
	y1=get_spectrum(par,n)[p]
	deriv=(y2-y1)/(2*dx)
	return x,y,deriv
			
for i in range(6):
	x,y,dy = fun(pars_cur,i)
	x = x/pars_cur[i]
	dy = dy*pars_cur[i]
	plt.figure(figsize=(8,6))      
	plt.title('Parameter no.'+str(i+1))
	plt.xlim(0.998, 1.002)
	plt.xlabel('scaled parameter value, factor='+str(pars_cur[i]))
	plt.grid(alpha=0.75)
	plt.ylabel('y-value (Power spec.)')
	plt.scatter(x,y,label='Points from function')
	plt.plot([min(x),max(x)],[y[5]+dy*(min(x)-x[5]),y[5]+dy*(max(x)-x[5])], label='Fit computed using slope')
