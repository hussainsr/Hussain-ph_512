...
snr_l = np.abs(mf_l)/np.std(mf_l[np.argmax(abs(mf_l))+100:30000]) # Using a smaller chunck to calc. std to
snr_h = np.abs(mf_h)/np.std(mf_h[np.argmax(abs(mf_h))+100:30000]) # avoid the signal peak and window edge
...
psc_h = np.cumsum(np.abs(sN_h))/np.sum(np.abs(sN_h)) # PS cumulative livingston (fraction)
psc_l = np.cumsum(np.abs(sN_l))/np.sum(np.abs(sN_l))
...
plt.xlabel('freq')    
#	plt.xscale('log')
plt.plot(freq,tpsc_h,label='Hanford')
plt.plot(freq,tpsc_l,label='Livington')
plt.legend()
plt.show();plt.close()
